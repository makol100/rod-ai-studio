...
1. "SERIA TOMEK I JANUSZ" (lub dowolny odcinek duetu) → DROGA ROLKA HUMOR,
   aktualnie wersja 3.4 (zawsze najwyższa wersja wiedza/DROGA_ROLKA_HUMOR.md).
   Wideo z dialogiem przez Veo (fal-ai/veo3.1/lite): kanon verbatim → LOCK →
   biblioteka postaci → kadry+bramka A → oko Tomasza → kanarek → preflighty →
   Veo (ang. reżyseria + polska kwestia w cudzysłowie) → strażnik → montaż →
   ocena natywna. BIELIK NIE UCZESTNICZY.

2. "CZYSTY BIELIK" → rolki TEMATYCZNE + generowanie ZDJĘĆ (pipeline foto):
   Bielik pisze polski tekst → FLUX/obrazy → lektor Marek (edge-tts) → Ken Burns
   → montaż. Tryb API tryb_jezykowy="czysty_bielik". To rolki ogrodnicze/porady
   na kanał ROD, NIE humor. Veo tu nie ma.

Komenda "seria Tomek i Janusz" = z automatu droga 1. "Czysty Bielik / rolka
tematyczna / zdjęcia" = z automatu droga 2. Bez dopytywania którą drogą.
# ====================================================================
...
## Sesja 21/22.07.2026 — KRYSTYNA (greenworks-optimow) OPUBLIKOWANA na GitHub
**Kontekst:** poprzednia sesja padła (fabryka + HA MCP "Tool not found") PRZED pushem repo Krystyny. Repo czekało zbudowane i zweryfikowane w /root/greenworks-optimow.
**Fundament (przed akcją):** tool_search 4× zwracał tylko telefon+Chrome → potwierdzony brak fabryki/HA. Termius na Fold7 pusty (brak hosta/klucza — roboczy SSH to S24 Ultra). Zgłoszone Tomaszowi; on przełączył konektor → fabryka + HA DZIAŁKA wróciły.
**Stan repo zastany (sprawdzony, nie zgadany):** git init + add + commit JUŻ wykonane przed crashem — branch main, commit 6006c63 "greenworks_optimow: initial HACS release v1.0.0", tree clean. Brakowało TYLKO remote+push. (Pamięć notowała "zostało git init/add/commit/push" — sprawdzenie na miejscu pokazało, że proces zaszedł dalej. Zawsze czytać realny stan.)
**Weryfikacja zawartości:** 9×.py kompilują się (py_compile OK), manifest v1.0.0 codeowners @makol100 config_flow true, const.py ma GUC_CLIENT_SECRET, hacs.json OK, __pycache__ poprawnie ignorowany (nie w tracku), 14 plików tracked.
**Akcja:** git remote add origin git@github.com:makol100/greenworks-optimow.git → git push -u origin main → [new branch] main. ZWERYFIKOWANE: git ls-remote origin HEAD = lokalny HEAD = 6006c63; status main...origin/main bez rozjazdu. Repo publiczne żyje.
**ZOSTAŁO (krok 2 planu — wdrożenie na Działkę):** HACS custom-repo add (makol100/greenworks-optimow) → restart HA Działka → konfiguracja (email/hasło Greenworks w UI — AKCJA TOMASZA) → podgląd/mirror stanu na HA Dom bez drugiego logowania do chmury (token Działki podaje Tomasz w sesji, NIE w pamięci) → usunięcie starego wpisu integracji z HA Dom.
**Lekcja:** fundament-check (tool_search + ekran telefonu) wyłapał martwe MCP ZANIM próbowałem pchać w próżnię — zamiast raportować fałszywe "gotowe".


## Sesja 26.07.2026 — STANOWISKO DWUMODELOWE (Claude Code + Codex) NA VPS ✔
**Zainstalowane i ZWERYFIKOWANE:** Claude Code 2.1.220 (zalogowany kontem Max Tomasza), Codex CLI 0.145.0 (zalogowany ChatGPT Plus → pracownik = GPT-5.6 Sol), oficjalny plugin OpenAI codex@openai-codex 1.0.6 (enabled; /codex:setup cały zielony, auth verified). Skill **/route** w /root/.claude/skills/route/SKILL.md: pętla SZEF (Claude — przepytuje, planuje, recenzuje) → SĘDZIA (/codex:adversarial-review weryfikuje plan) → PRACOWNIK (Codex/Sol buduje przez /codex:rescue). Sesja tmux `claw` żywa na VPS.
...
==============================================================================
## SESJA 07.08.2026 13:01 CEST
==============================================================================

SESJA 6-7.08: (1) ROUTER SOSNOWCA przenumerowany 192.168.0.1->192.168.50.1 (Tomasz fizycznie na miejscu), trasa Tailscale 192.168.50.0/24 przez tools/mcp_wybickiego.py (advertise Supervisor, param 'options', potem restart) + zatwierdzona w panelu Tailscale przez telefon Fold7 -> kolizja z Dzialka ROZWIAZANA, obie lokacje naraz, panel .50.1 z VPS HTTP 200. (2) TELEFON: sterowanie = apka Android Remote Control MCP port 8080 (connector Telefon Fold7), NIE ADB - port ADB ROTUJE przy zmianie WiFi = slepa uliczka (bylo w telefon-mcp.md, stracono duzo czasu na ADB/nmap/cookies zanim Tomasz wskazal Genka). (3) WIDEO/YOUTUBE: yt-dlp z VPS zablokowany (bot detection), web_fetch 429, cookies HttpOnly - JEDYNA droga to tools/oczy_uszy.py: Gemini oglada przez fileUri (omija blokade botow). 2 transkrypcje zrobione od reki. (4) OCENY REPO: GitHub Trending #124 - z 24 repo NIC nie wchodzi (im-not-ai koncept, Tomasz odrzucil). TencentDB Agent Memory - CALOSC NIE (adapter Hermesa 3 bugi wg issues, Henio znalazl), KONCEPT TAK. (5) ZBUDOWANO WLASNY STAN: tools/pamiec_stan.py - pamiec krotkotrwala (TRZECIA warstwa obok teleport/wiedza), graf Mermaid+node_id+drill-down, offload do refs/, HYBRYDA (surowe=Hans/most.jsonl przez --zrodlo-most, Klaudek dodaje wezly znaczeniowe), przestrzenie per agent. Commity 59e52b5+3383d06, 3 testy zielone (zweryfikowane przez Klaudka). Wdrozony: protokol startu (4 czlon 'pamiec_stan.py pokaz') + pamiec Klaudka. (6) DEKRET Tomasza: 'Zawsze zapisuj i sie nie pytaj' - do teleportu automatycznie, bez pytania.


==============================================================================
## SESJA 07.08.2026 14:18 CEST
==============================================================================
