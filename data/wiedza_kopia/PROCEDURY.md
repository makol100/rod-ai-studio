...
- Powiadomienia Telegram z VPS: tools/tg_powiadom.sh "tekst" [url] → POST webhook
  fabryka_rolka_gotowa (HA Dom, payload {"video_url","caption"}); checkpointy:
  webhook fabryka_checkpoint_gotowy {"wiadomosc"}. HTTP 200 ≠ dostarczone —
  potwierdzenie zawsze na telefonie Tomasza. Gdy HA Dom leży, telegram pada
  (log 10004) — publikacja MA iść dalej mimo błędu powiadomienia (nigdy blokada).
- Po każdej publikacji FB (Graph API) wysłać tg_powiadom: tytuł + permalink reela.
