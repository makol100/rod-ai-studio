...
segs,_=m.transcribe('/root/rod-ai-studio/data/rolki_info/tauron_kdt/tomasz/_tmp.wav', language='pl', word_timestamps=True, beam_size=5)
import json
out=[]; words=[]
for s in segs:
...
def norm(s):
    ...
    w=re.findall(r"[a-ząćęłńóśźż0-9]+", s)
    # 02.09: koncowe 'ę' wymawiane jak 'e' (norma), '991' = slownie
    out=[]
    for x in w:
        x=re.sub(r'ę$','e',x)
        if x=='991': out+=['dziewięćset','dziewięćdziesiąt','jeden']; continue
        out.append(x)
    return out
