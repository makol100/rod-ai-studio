...
iu = fal_client.upload_file('/root/rod-ai-studio/data/wiadomosci/alejka2/izabela/izabela_stoi_v2.jpg')
au = fal_client.upload_file(f'{B}/izabela_{n}.mp3')
h = fal_client.submit(M, arguments={'image_url': iu, 'audio_url': au, 'prompt': PROMPT})
json.dump({'rid': h.request_id, 'model': M}, open(f'{B}/_kling_{n}_v2_state.json', 'w')); print('SUBMIT', n, h.request_id[-12:], flush=True)
for i in range(150):
    time.sleep(10)
    try:
        st = fal_client.status(M, h.request_id, with_logs=False)
        if 'Completed' in type(st).__name__:
            r = fal_client.result(M, h.request_id); json.dump(r, open(f'{B}/_kling_{n}_v2_resp.json', 'w'))
            url = r.get('video', {}).get('url') if isinstance(r.get('video'), dict) else r.get('video_url') or r.get('url')
            urllib.request.urlretrieve(url, f'{B}/izabela_{n}_v2.mp4'); print('OK', n, flush=True); break
