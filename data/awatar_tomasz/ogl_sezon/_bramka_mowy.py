...
segs,_=m.transcribe('/root/rod-ai-studio/data/awatar_tomasz/ogl_sezon/_tmp.wav', language='pl')
import json; print(json.dumps([(round(s.start,1),round(s.end,1),s.text.strip()) for s in segs],ensure_ascii=False))
'''
...
tr=json.loads(out); usl=' '.join(t for _,_,t in tr)
def norm(s):
    s=s.lower().replace('5 września','piątego września').replace('godzina 17','godzina siedemnasta')
