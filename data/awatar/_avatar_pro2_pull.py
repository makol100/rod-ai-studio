...
st = json.load(open(f'{B}/_avatar_pro2_state.json'))
if os.path.exists(f'{B}/stanislaw_avatar_pro2_raw.mp4'):
...
json.dump(res, open(f'{B}/_avatar_pro2_resp.json', 'w'))
url = (res.get('video') or {}).get('url')
if not url:
    print('BRAK URL:', json.dumps(res)[:250]); sys.exit(1)
subprocess.run(['curl', '-sL', '-o', f'{B}/stanislaw_avatar_pro2_raw.mp4', url], check=True)
print('POBRANY: stanislaw_avatar_pro2_raw.mp4')
