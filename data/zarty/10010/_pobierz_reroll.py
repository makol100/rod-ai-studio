"""Pobranie 4 klipow re-rollu po RID (gen_state_reroll_*.json) -> klip_kXX_reroll.mp4 (+faststart podglad na panel)."""
...
gotowe, czekamy = [], []
for g in sorted(glob.glob(f'{B}/gen_state_reroll_*.json')):
    n = g.split('reroll_')[1].split('.')[0]
    s = json.load(open(g))
    ...
    url = res['video']['url']
    out = f'{B}/klip_{n}_reroll.mp4'
    r = requests.get(url, timeout=180); r.raise_for_status()
    open(out, 'wb').write(r.content)
    subprocess.run(['ffmpeg','-y','-loglevel','error','-i',out,'-c','copy',
                    '-movflags','+faststart', f'{P}/10010_klip_{n}_reroll.mp4'], check=True)
    gotowe.append(n)
