# INDEKS WIEDZY FABRYKI

Wygenerowany automatycznie przez `tools/porzadek.py` — 29.07.2026 07:00.
NIE EDYTOWAC RECZNIE: kazde uruchomienie skryptu nadpisuje ten plik stanem dysku.

Szukanie tresci: `python3 tools/szukaj.py <slowo>` — przeszukuje wszystko ponizej plus teleporty.

## Pliki wiedzy (35), od najswiezszego

| plik | zmiana | rozmiar | o czym |
|---|---|---|---|
| `NAUKI.md` | 29.07 | 12K | NAUKI — lekcje z produkcji (destylat) |
| `PAMIEC_INFRASTRUKTURA.md` | 29.07 | 12K | PAMIĘĆ KLAUDKA — INFRASTRUKTURA (eksport 29.07.2026) |
| `POMIAR_WD0001_KASKADA.md` | 29.07 | 1K | POMIAR KASKADY WD_0001 v1→v6 (29.07.2026) — przyrząd, nie opowieść |
| `LEKCJA_0009_henik_konfabulacja.md` | 29.07 | 10K | LEKCJA 0009 — HENIK SFABRYKOWAŁ CAŁĄ ANALIZĘ (29.07.2026) |
| `SKILLE_WYBOR.md` | 28.07 | 3K | WYBÓR SKILLI — z przeczytanego kodu, nie z opisów (28.07.2026) |
| `PILOTAZ_RUVIEW.md` | 28.07 | 5K | PILOTAŻ RuView (WiFi CSI) — decyzja i plan (28.07.2026) |
| `DECYZJE_0008_ruview_dzialka.md` | 28.07 | 2K | DECYZJA 0008 — RuView (WiFi CSI) IDZIE NA HA DZIAŁKA (28.07.2026) |
| `ANALIZA_10_REPO.md` | 28.07 | 3K | ANALIZA 10 REPOZYTORIÓW AI (28.07.2026) |
| `DECYZJE_0007_bramka_oka.md` | 28.07 | 1K | DECYZJA 0007 — BRAMKA OKA fail-closed (28.07.2026, dekret Tomasza "Naprawiać") |
| `NAUKI_SERII.md` | 28.07 | 20K | 2026-07-25 (#10009, kosztowała $5.12) | ŻELAZNA ZASADA KODU PRODUKCYJNEGO: każdy skrypt wykonujący PŁATNE oper |
| `LISTA_PRACOWNIKOW.md` | 28.07 | 2K | LISTA PRACOWNIKÓW FABRYKI — załoga stała |
| `DECYZJE_AWATAR.md` | 28.07 | 26K | DECYZJE — PROJEKT AWATAR "DZIAŁKOWY DZIENNIKARZ" |
| `DECYZJE_CLAUDE_CODE.md` | 28.07 | 5K | 2026-07-26 | "Leć" (po planie: Claude Code + Codex CLI + plugin codex@openai-codex na fabryce, review gate OFF |
| `PORADNIK_17_CLAUDE_CODE.md` | 27.07 | 3K | PORADNIK: 17 porad praktyka o pracy z Claude Code |
| `STANISLAW_CANON_1.0.md` | 27.07 | 4K | STANISLAW_CANON_1.0 — BIBLIA PANA STANISŁAWA |
| `BIBLIA_STANISLAWA.md` | 27.07 | 6K | BIBLIA PANA STANISŁAWA — DRAFT do zatwierdzenia punkt po punkcie |
| `ZALOGA_LIMITY.md` | 27.07 | 1K | LIMITY ZAŁOGI (badanie Zenka 27.07.2026, źródła: oficjalne docs OpenAI/Google) |
| `DECYZJE_10010.md` | 26.07 | 45K | DECYZJE — odcinek #10010 "Kiedy złapiesz złodzieja jabłek" (remake Niemowy) |
| `PRZEKAZANIE_2026-07-26_10010.md` | 26.07 | 4K | PRZEKAZANIE OKNA — 2026-07-26 — odcinek #10010 (+ zamknięcie 10009) |
| `AKTYWA_SERII.md` | 26.07 | 1K | KSIĘGA AKTYWÓW SERII HUMOR |
| `DECYZJE_10009.md` | 26.07 | 10K | DECYZJE — odcinek #10009 "Działkowy czarny rynek (Wymiana nawozu)" |
| `DECYZJE_SERIA_HUMOR.md` | 25.07 | 13K | DECYZJE TOMASZA — SERIA HUMOR (obsada, zasady ponadodcinkowe) |
| `KANON_10009.md` | 25.07 | 2K | KANON odcinka #10009 — "Działkowy czarny rynek (Wymiana nawozu)" |
| `DECYZJE_000098.md` | 25.07 | 2K | DECYZJE — rolka foto #000098 (stacja pogodowa) |
| `PROMPTY_WZORCE.md` | 23.07 | 3K | BANK PROMPTÓW-ZWYCIĘZCÓW — seria Tomek i Janusz (DROGA HUMOR) |
| `DECYZJE_10008_kontrola.md` | 23.07 | 2K | DECYZJE — #10008 "Kontrola zza płotu" |
| `DROGA_ROLKA_HUMOR.md` | 23.07 | 14K | DROGA ROLKA HUMOR 3.5 |
| `PROCEDURY.md` | 18.07 | 3K | PROCEDURY — jak się robi |
| `DECYZJE_10007_jablko.md` | 18.07 | 12K | DECYZJE — #10007 „Zasada spadającego jabłka" |
| `DECYZJE_10005_slimak.md` | 18.07 | 4K | DECYZJE TOMASZA — odcinek #10005 "Operacja: Ślimak nagi" |
| `DECYZJE_10006_afera.md` | 17.07 | 6K | DECYZJE — #10006 "Afera elektryczna na RODOS" |
| `DECYZJE_10004.md` | 17.07 | 1K | DECYZJE TOMASZA — odcinek #10004 "Niemowa" |
| `ARCHIWUM_DROGA_HUMOR_v1.md` | 16.07 | 2K | DROGA ROLKA HUMOR |
| `STYL.md` | 15.07 | 2K | STYL — treść i publikacja |
| `ARCHITEKTURA.md` | 15.07 | 4K | ARCHITEKTURA — jak jest teraz |

## Pozostale magazyny

| plik | rozmiar | zmiana | rola |
|---|---|---|---|
| `/root/rod-ai-studio/TELEPORT_fabryka.md` | 127K | 26.07 | teleport fabryki — ARCHIWUM, nie czytac w calosci, szukac przez szukaj.py |
| `/root/TELEPORT_HA.md` | 42K | 19.07 | teleport HA — ARCHIWUM, jw. |
| `/root/rod-ai-studio/AGENTS.md` | 3K | 29.07 | regulamin pracy agentow w repo — CZYTAC ZAWSZE |
| `/root/.claude/CLAUDE.md` | 1K | 26.07 | konfiguracja Claude Code na VPS |
| `/home/hermes/PODRECZNIK_DYZURNEGO.md` | 7K | 29.07 | podrecznik Henika (dyzurny) |

## Zasada

Jeden fakt ma jedno miejsce. Nowy zapis idzie do `wiedza/`, nie do teleportu.
Teleporty sa ARCHIWUM historycznym — czyta sie je wyszukiwarka, nie w calosci.
