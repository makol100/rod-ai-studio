...
## Bramka ukończenia (tools/zrobione.py) — przebieg 18.09
- Run 1: Henio POTWIERDZAM (własny dekod cv2 z PNG); Zenek/Genek BRAK ŚLADU — słusznie: test wyłączał arkusz@72 dpi (0/4), a twierdzenie mówiło o 72 dpi → poprawka: arkusz cięty na 4 ćwiartki jak nożyczkami, dekod pojedynczy, 4/4 w 300/100/72; test bez wyjątków.
- Run 2: cała trójka BRAK ŚLADU — słusznie: twierdzenie „ślad wysyłki w data/wyslane_tomaszowi.jsonl", a plik nie istniał (dziennik dopisany do wyslij_tomaszowi.py PO wysyłce v3). Lekcja Klaudka: nie wpisywać do twierdzenia bramki rzeczy, których ślad dopiero powstanie. Odbiór v3 potwierdził sam Tomasz („Dzięki super" 08:35).
- Run 3: twierdzenie ograniczone do części technicznej (dekodowanie + geometria + test).
