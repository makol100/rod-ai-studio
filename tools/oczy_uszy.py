...
# 4.08: oczy chodzily na gemini-2.5-flash, choc Genek pracuje na rodzinie 3.x — tym samym kluczem.
# Dekret Tomasza: "Heniek ma dostep do modelu Gienka, ktory jest wlasnie wykorzystywany".
...
def patrz(zrodlo: str, polecenie: str, k: str, limit: int) -> str:
    ...
    body = json.dumps({
        ...
        # 4.08: "thinkingConfig: thinkingBudget 0" to ustawienie rodziny 2.5 (wylacza rozumowanie
        # dla oszczednosci). Modele 3.x ODRZUCAJA je bledem INVALID_ARGUMENT — zmierzone tego dnia,
        # gdy oczy podniesiono z 2.5-flash na 3.6-flash. Dokladamy je tylko tam, gdzie dziala.
        "generationConfig": ({"temperature": 0.0, "maxOutputTokens": limit}
                             if not MODEL.startswith("gemini-2.")
                             else {"temperature": 0.0, "maxOutputTokens": limit,
