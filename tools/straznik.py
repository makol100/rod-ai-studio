...
def straznik_techniczny(plik, exp_w, exp_h, freeze_ok=False, ciecia_ok=None):
    info = _ffprobe(plik)
    if "blad" in info:
    ...
    if ciecia_ok is None:
        if ciecia > 0: problemy.append(f"wykryto {ciecia} cięć sceny wewnątrz klipu")
    elif ciecia > ciecia_ok:
        problemy.append(f"cięcia OBCE: {ciecia} > plan {ciecia_ok}")
    elif ciecia < ciecia_ok:
        warny.append(f"detektor widzi {ciecia}/{ciecia_ok} cięć (jednorodne ciemne sceny)")
    st = "FAIL" if problemy else ("WARN" if warny else "PASS")
    return {"status": st, "detale": {**info, "problemy": problemy, "uwagi": warny}}
...
def straznik_napisow(plik, n=8, tekst_dozwolony=False):
    ...
    with tempfile.TemporaryDirectory() as td:
        for p in _klatki(plik, n, td):
            try:
                d = pytesseract.image_to_data(Image.open(p), lang="pol+eng",
                                              output_type=pytesseract.Output.DICT)
            except Exception as e:
                return {"status": "POMINIĘTY", "detale": f"tesseract: {e}"}
            slowa = [t.strip() for i, t in enumerate(d["text"])
                     if len(t.strip()) >= 3 and int(d["conf"][i]) >= 50
                     and re.fullmatch(r"[A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż]{3,}", t.strip())]
            if len(slowa) >= 2:
                znaleziska.append({"klatka": p.name, "tekst": " ".join(slowa)[:80]})
    if tekst_dozwolony:
        st = "PASS" if znaleziska else "FAIL"  # kontrola pozytywna (np. plansza)
    elif len(znaleziska) >= 2:
        st = "FAIL"  # tekst TRWAŁY (≥2 klatki) = napisy-widmo
    elif znaleziska:
        st = "WARN"  # jednoklatkowy pik = szum seeka/kompresji, nie napis
    else:
        st = "PASS"
...
def straznik_scenariusza(plik, kwestia):
    ...
    segs, info = m.transcribe(str(plik))  # BEZ wymuszania jezyka — detekcja
    czas_pliku = _ffprobe(plik).get("czas", 1e9)
    segs = [x for x in segs if x.start < czas_pliku - 0.25]  # halucynacje za EOF precz
    uslyszane = " ".join(s.text.strip() for s in segs).strip()
    jezyk = getattr(info, "language", "?")
    pewnosc = round(float(getattr(info, "language_probability", 0.0)), 2)
    if jezyk != "pl":
        return {"status": "FAIL", "detale": {"język_lektora": jezyk,
                "pewność": pewnosc, "usłyszane": uslyszane[:120],
                "problem": "lektor mówi w złym języku (wymagany pl)"}}
    ...
    return {"status": st, "detale": {"oczekiwane": kwestia,
                                     "usłyszane": uslyszane, "zgodność": round(ratio, 2), "język": jezyk, "pewność_języka": pewnosc}}
...
def straznik_tozsamosci(plik, wzorce, n=10, prog=0.35):
    ...
    obcy, male, graniczne, trafienia = [], [], [], {k: [] for k in emb_w}
    with tempfile.TemporaryDirectory() as td:
        for p in _klatki(plik, n, td):
            for tw in _twarze(p):
                ...
                elif sims[naj] >= prog - 0.05:
                    graniczne.append({"klatka": p.name, "najbliżej": naj,
                                      "sim": round(sims[naj], 2)})
                else:
    ...
    det["graniczne_WARN"] = graniczne
    st = "FAIL" if obcy else ("WARN" if (male or graniczne) else "PASS")
    return {"status": st, "detale": det}
...
def main():
    ...
    ap.add_argument("--final", type=int, default=None,
                    help="tryb finału: oczekiwana liczba cięć montażowych; włącza freeze_ok")
    ...
    R = {
        "techniczny":  straznik_techniczny(plik, a.exp_w, a.exp_h,
                            a.freeze_ok or (a.final is not None), ciecia_ok=a.final),
