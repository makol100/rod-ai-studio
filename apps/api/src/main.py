...
async def auth_mutacji(request: Request, call_next):
    if request.method in ("POST", "PUT", "DELETE", "PATCH") and request.url.path not in ("/kamery/login", "/kontakt/wyslij", "/tablica/wyslij"):  # /kamery/login = formularz logowania dzialkowcow (03.09), sam sprawdza haslo
...
from src.tablica_rod import router as tablica_rod_router
from src.radar_rod import router as radar_rod_router
app.include_router(zarty_router)
app.include_router(kamery_auth_router)
app.include_router(kontakt_rod_router)
app.include_router(tablica_rod_router)
app.include_router(radar_rod_router)
...
def mapa():
    from fastapi.responses import HTMLResponse
    with open("/app/src/mapa.html", encoding="utf-8") as f:
        return HTMLResponse(f.read())
...
def n150_install():
    ...
    with open("/app/src/n150.sh", encoding="utf-8") as f:
        return PlainTextResponse(f.read())


from fastapi.staticfiles import StaticFiles
app.mount("/n150files", StaticFiles(directory="/root/rod-ai-studio/data/n150files"), name="n150files")


@app.get("/n150.apkovl.tar.gz")
def n150_apkovl():
    from fastapi.responses import FileResponse
    return FileResponse("/app/src/n150.apkovl.tar.gz", media_type="application/gzip")


@app.get("/ipxe.efi")
def n150_ipxe():
    from fastapi.responses import FileResponse
    return FileResponse("/app/src/n150_ipxe.efi", media_type="application/octet-stream", filename="netboot.xyz.efi")


@app.get("/n150fix")
def n150_fix():
    from fastapi.responses import PlainTextResponse
    with open("/app/src/n150fix.sh", encoding="utf-8") as f:
        return PlainTextResponse(f.read())
