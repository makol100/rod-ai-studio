...
TLO, DZIALKA, DZIALKA_OB = "#fffdf6", "#eaf3e4", "#c3d9b8"
ALEJKA, TEKST, TEKST_2 = "#e6ddc4", "#1f3a29", "#5e6b54"
LINIA, DROGA, DROGA_L = "#c9c3b4", "#b6b099", "#fffdf6"
...
RZEDY = [
    ...
    (Y_R4, ["PARK", "DOM", "PARK", 33, 32, 31, 30, 29, 28]),
...
def rysuj_baze(tytul, podtytul, przed_dzialkami=None):
    ...
    d.polygon([(X0, Y_MAPA + PARK2_H), (X_KON, Y_MAPA + PARK2_H),
               (X_KON, Y_MAPA - 2), (X0, Y_MAPA + 40)], fill=PARK_KOL, outline=PARK_OB)
    ...
    for y0, rzad in RZEDY:
        ...
        while i < len(rzad):
            ...
            if p == "DOM":
                i += 1
                continue  # Dom rysowany na scalonym placu
            xa, xb = x_kol(i)
            if p == "PARK":
                pass  # parking rysowany jako JEDEN scalony blok nizej
            else:
                d.rectangle([xa, y0, xb, y0 + DZ_H], fill=DZIALKA, outline=DZIALKA_OB, width=3)
                srodek(d, ((xa + xb) / 2, y0 + DZ_H / 2), str(p), F_NUM, TEKST)
            i += 1

    # SCALONY PLAC (parking nr 1): jeden obszar — kolumny 0-2 rzedu R4 + pas alejki srodkowej pod nim
    _xa, _ = x_kol(0)
    _, _xb = x_kol(2)
    d.rectangle([_xa, Y_R4, _xb, Y_ALS + AL_H], fill=PARK_KOL, outline=PARK_OB, width=3)
    # miejsca parkingowe (kreseczki) jak na wzorze
    for _k in range(6):
        _px = _xa + 14 + _k * ((_xb - _xa - 28) / 6)
        d.rectangle([_px, Y_ALS + 12, _px + 22, Y_ALS + 26], fill="#d4c176")
    d.text((_xa + 12, Y_R4 + 6), "PARKING NR 1", font=F_BLOK, fill="#5e6b54")
    # DOM na wierzchu placu (kolumna 1)
    _dxa, _ = x_kol(1)
    _k2a, _k2b = x_kol(2)
    _dxb = _k2a + (_k2b - _k2a) / 2   # dom siega do POLOWY kolumny nad dzialka 36
    d.rectangle([_dxa, Y_R4, _dxb, Y_R4 + DZ_H], fill=DOM, outline=DOM_OB, width=3)
    d.ellipse([_dxa + 10, Y_R4 + 8, _dxa + 38, Y_R4 + 36], outline="#c0392b", width=2)
