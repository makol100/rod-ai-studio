...
export class VideoRTC extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (this.video) {
            ...
            if (seek.length > 0) {
                this.video.currentTime = Math.max(seek.start(0), seek.end(seek.length - 1) - 3);
            }
            this.play();
        } else {
            this.oninit();
        ...
        this.onmessage['mse'] = msg => {
            ...
            sb.addEventListener('updateend', () => {
                ...
                if (!sb.updating && sb.buffered && sb.buffered.length) {
                    ...
                    const start = end - 12;   // wieksze okno bufora (bylo 5)
                    const start0 = sb.buffered.start(0);
                    if (start > start0) {
                        sb.remove(start0, start);
                        ms.setLiveSeekableRange(start, end);
                    }
                    // ROD: spokojny bufor ~3 s za zywym, normalna predkosc; skok tylko gdy opoznienie ucieklo
                    const cel = end - 3;
                    if (this.video.currentTime < start) this.video.currentTime = cel;
                    const gap = end - this.video.currentTime;
                    if (gap > 8) { this.video.currentTime = cel; this.video.playbackRate = 1.0; }
                    else if (gap < 0.5) this.video.playbackRate = 0.95;   // za blisko zywego -> lekko zwolnij, nie zacinaj
                    else this.video.playbackRate = 1.0;
